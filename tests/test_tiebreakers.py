...
def test_clean_standings_no_ties():
    ...
    rng = random.Random(0)
    order = resolve_standings(matches, rng=rng)
...
def test_complete_tie_falls_back_to_random_lots():
    ...
    order1 = resolve_standings(matches, rng=rng1)
    order2 = resolve_standings(matches, rng=rng2)
    # Same seed → same order
    assert order1 == order2
    # Different seed → likely different order (with 4! = 24 perms, p(same) ≈ 1/24)
    rng3 = random.Random(7)
    order3 = resolve_standings(matches, rng=rng3)
