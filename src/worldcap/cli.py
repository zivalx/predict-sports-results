...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="worldcap")
...
async def _run_refresh(trigger: str) -> int:
    from worldcap.api.app import _default_clients
...
def _run_serve(host: str, port: int, reload: bool) -> int:
    import uvicorn
    uvicorn.run("worldcap.api.app:app", host=host, port=port, reload=reload)
    return 0
