...
Posts the worldcap daily digest to a configured WhatsApp group. Fire-once
...
- Stores WhatsApp session under its own `clientId="worldcap"` namespace
- Targets a different group (configurable)
- Doesn't share any config, state, or process with the existing bot

You can run both bots on the same WhatsApp account by linking the worldcap
...
npm install
cp .env.example .env
# Edit .env — set WORLDCAP_GROUP_NAME to the exact group name from
...
The session is then cached under `.wwebjs_auth/session-worldcap/` and
...
```cron
0 9 * * * cd /opt/worldcap/wa-bot && /usr/bin/npm start >> /var/log/worldcap-wa.log 2>&1
```
...
2. Connects to WhatsApp with `clientId="worldcap"`.
3. Finds the group named `WORLDCAP_GROUP_NAME`.
...
- **Conflict with existing bot**: shouldn't happen — different `clientId`
  ...
  `.wwebjs_auth/session-worldcap/` and re-scan QR.
